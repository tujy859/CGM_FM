...
| 数据集 | 人数 | 角色 | 清洗要点 |
|---|---|---|---|
| ShanghaiT1DM/T2DM | 12+100 | 预训练+评估 | xlsx 无表头读 A/B 列；Excel 序列日期；HbA1c mmol/mol ÷10.93；血脂 mmol/L ×38.67；胰岛素 pmol/L ÷6.945 |
| Colas_DFA | 208 | 预训练 | 无日期按行序重建 5min 时间轴；西语列名；T2D 随访标签 17/208 |
| Hall (TSV) | 57 | 评估 | 纯 CGM；glucotype 按 Hall 2018 自算 |
| BIG IDEAs | 16 | 预训练 | Dexcom 导出格式，过滤 `Event Type == EGV` |
| CGMacros | 45 | 评估（标签最全） | 1min 插值网格，只取原始 5/15min 采样点；bio.csv 是标签主来源 |
| CGM-JEPA-Pretraining (HF) | 228 | 预训练 | 现成三列 CSV，直接并入 |
...
### 1.6 语料划分（subject-disjoint 铁律）

- 预训练池（开放部分）：Colas 208 + CGM-JEPA-Pretraining 228 + Shanghai 77 + CGMacros 15 + BIG IDEAs 16 + 8 个新开放集 ~164 ≈ **700+ 人**（远超论文 477）
